...
int main(void){
    ...
    if (YSOK != pacman_death.LoadWav("pacman_death.wav") &&
        YSOK != pacman_death.LoadWav("music/pacman_death.wav"))
    {
        printf("Error!  Cannot load pacman_death.wav!\n");
    }
    if (YSOK != scene.LoadWav("scene.wav") &&
        YSOK != scene.LoadWav("music/scene.wav"))
    {
        printf("Error!  Cannot load scene.wav!\n");
    }
    if (YSOK != cherry.LoadWav("pacman_eatfruit.wav") &&
        YSOK != cherry.LoadWav("music/pacman_eatfruit.wav"))
    {
        printf("Error!  Cannot load pacman_eatfruit.wav!\n");
    }
    if (YSOK != areyouready.LoadWav("areyouready.wav") &&
        YSOK != areyouready.LoadWav("music/areyouready.wav"))
    {
        printf("Error!  Cannot load pacman_eatfruit.wav!\n");
    }
    while(key != FSKEY_ESC){
